<template>
  <div class="ships">
    <button v-on:click="toggleStats">Toggle Stats</button>
    <div v-show="json">
      {{ ships }}
    </div>
    <h2>Attacker</h2>
    ...
    <table>
      <thead>
        <th v-show="stats">Tank</th>
        <th v-show="stats">Shooter</th>
        <th>Order</th>
        <th>Ship</th>
        <th>Quantity</th>
        <th v-show="stats">Structure</th>
        <th v-show="stats">Armor</th>
        <th v-show="stats">Shield</th>
        <th v-show="stats">Rocket</th>
        <th v-show="stats">Bullet</th>
        <th v-show="stats">Laser</th>
        <th>Survivor</th>
      </thead>
      <tbody>
        <tr v-for="(attacker, index) in attackers" :key="attacker.id">
          <td v-show="stats && attacker.ship !== 'end'">
          ...
          <td v-show="attacker.ship !== 'end'">
            <select v-model="attacker.ship">
              <option
                v-for="ship in filteredShips(attacker.type)"
                :key="ship.id"
                :value="ship.name"
                >{{ ship.name }}</option
              >
    ...
    <p>
      Result:
      <font v-if="currentAttacker === this.slots" color="green"
        ><b>Winner</b></font
      ><font v-if="currentDefender === this.slots" color="red"
        ><b>Looser</b></font
      >
    </p>
    <table>
        ...
        <tr v-for="(defender, index) in defenders" :key="defender.id">
          ...
          <td v-show="defender.ship !== 'end'">
            <select v-model="defender.ship">
              <option
                v-for="ship in filteredShips(defender.type)"
                :key="ship.id"
                :value="ship.name"
                >{{ ship.name }}</option
              >
    ...
    <h2>Battle</h2>
    <button v-on:click="prepare(battleStyle)">Prepare</button>
    <button v-on:click="battle(battleStyle)">Play Round</button>
    <button v-on:click="auto(battleStyle)">Battle</button>
    <h2>Battle Log</h2>
    <p>{{ result }}</p>
    <p>Next Turn: {{ turn }}</p>
    <p>Round: {{ round }}</p>
...
export default {
  ...
  methods: {
      ...
      if (battleStyle === 1) {
        ...
        if (
          ...
          if (this.defenders[defenderIndex].structure === 0) {
            this.result = "Attacker won round";
          }
          if (this.attackers[attackerIndex].structure === 0) {
            this.result = "Defender won round";
          }
    ...
    filteredShips(shipType) {
      return this.ships.filter(s => s.type === shipType);
    },
    up(who, ship) {
      if (who === "attacker") {
